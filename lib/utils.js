...
const getTemplateData = (name,dirPath)=>{
  // src/views/login/ login  1;  src/views/main/example/ example  2;
  // src/store/modules/login/ login  1;  src/store/modules/main/example/ example  2;
  // src/service/login/ login  1;  src/service/main/example/ example  2;
  let pathDir = dirPath+'/'+name
  pathDir = pathDir.split('/').filter((v)=>{return v!==''}) // [src,view,login,login]
  let reg = 'src/views/'
  if(dirPath.indexOf('src/views/')!==-1){
    reg = 'src/views/'
  } else if(dirPath.indexOf('src/store/modules')!==-1){
    reg = 'src/store/modules/'
  } else if(dirPath.indexOf('src/service/')!==-1){
    reg = 'src/service/'
  }
  let suffixDir = dirPath.split(reg).pop() // 
  // console.log('dirPath=',dirPath)
  // console.log('pathDir=',pathDir)
  // console.log('suffixDir=',suffixDir)
  return {
    // dirPath :getDirPath(name), // 新建组件的路劲
    dirPath, // 新建组件的路劲 src/views/ main/example
    suffixDir, //新建组件存放在src/views/目录下的路劲：main/example
    actionName:suffixDir.split('/').join('_'), //新建组件获取list的action name：main_example
