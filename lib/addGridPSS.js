...
/**
 * 新建一个页面对应的 gridview page, store and service 的指令
 * @param {} name 组件的名称 hello-world  helloworld helloWorld HelloWorld  Helloworld
 * @param {*} dir 新建组件存放的目录（该路径不能以/开头）
 */
const addGridPSS =  (name, dir)=>{
  // vue-temp-cli addGridPSS example -d src/views/main/example
  let pre = 'src/views/'
...
const addPageVue = (name, dir)=>{
  ...
  const templateFilePath = utils.resolveReallyPath('../template/src/views/main/gridview/gridview.vue.ejs')
...
const addPageRoute = (name, dir)=>{
  // 1.获取模板需要的数据
  const data = utils.getTemplateData(name, dir)
  const templateFilePath = utils.resolveReallyPath('../template/src/views/main/gridview/route.js.ejs')
...
const addPageConfig = (name, dir, config_file_name)=>{
  // 1.获取模板需要的数据
  const data = utils.getTemplateData(name, dir)
  const templateFilePath = utils.resolveReallyPath(`../template/src/views/main/gridview/page-config/${config_file_name}.js`)
